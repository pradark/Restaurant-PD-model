...
def plot_auc3(ax, y_train, y_train_pred, y_test, y_test_pred, y_val, y_val_pred, th=0.5):
    ...
    ax.plot(fpr_train, tpr_train, color="blue")
    ax.plot(fpr_test, tpr_test, color="red")
    ax.plot(fpr_val, tpr_val, color="green")
    ...
    train_text = 'Train KS = {:.2f}, AUC = {:.2f}'.format(ks_train, roc_auc_train)
    test_text = 'Test KS = {:.2f}, AUC = {:.2f}'.format(ks_test, roc_auc_test)
    val_text = 'OTV KS = {:.2f}, AUC = {:.2f}'.format(ks_val, roc_auc_val)
    no_skill_text = 'no skill'
    ax.legend([train_text, test_text, val_text, no_skill_text])
...
def exp_vs_act(df,score,target,title_text = None):
    ...
    ax = result.plot(x='grp', y=['act_event_rate','pred_event_rate'],color=['red', 'blue'],marker="*",title=title_text)
    ax.set_xticks(range(len(result)))
    p = ax.set_xticklabels([item for item in result.grp.tolist()],rotation=45)
    
    return result

def exp_vs_act_date(df,score,date,target):
    ...
    if str(df[score].dtype) !='object':
        pred_events = round(pd.DataFrame(df.groupby([date],dropna=False)[score].sum()),1).rename(columns={score: 'pred_events'})
        result = pd.merge(result, pred_events, how="left",on=date)
    ...
    result.plot(x=date, y=['act_event_rate','pred_event_rate'],color=['red', 'blue'],marker="*",ax=ax,title='Expected vs Actual')
    ax.set_xticks(range(len(result)))
    ax.set_ylabel("event_rate")
    p=ax.set_xticklabels([item for item in result[date].tolist()],rotation=90)

def bivariate(df,xdim,ydim,target):
    ...
    result['event_rate_%'] = round(result['events']/result['total']*100,2)
    ...
    #print(value_name1)
    #print(dfl2)
    title1 = str(ydim) + " (% dist, Event Rate) by " + str(xdim)
    my_colors = ['tab:blue', 'tab:orange', 'tab:green', 'tab:red', 'tab:purple', 'tab:brown', 'tab:pink', 'tab:gray', 'tab:olive', 'tab:cyan', 'linen']
    #my_colors = [(x/10.0, x/20.0, 0.75) for x in range(len(dfl2))] # <-- Quick gradient example along the Red/Green dimensions.
    ax = dfl2.T.plot(kind='bar', stacked=True, rot=1, figsize=(20, 10), title=title1, color=my_colors, alpha=0.6)
    ax.set_xticklabels(ax.get_xticklabels(), rotation=45,fontsize=14)
    ax.set_yticklabels(ax.get_yticklabels(), fontsize=14)
    legend_title = ydim
    ax.legend(bbox_to_anchor=(1.0, 1.0), title = legend_title)
    ax.set_ylabel('% Dist', fontsize=14)
    ax.set_xlabel(xdim, fontsize=14)
    ax.grid(False)
    plt.title(title1,fontsize = 18)

    # create a corresponding pivoted dataframe for survival rate %
    value_name2 = "Event Rate %"
    dfl3 = pd.melt(result, id_vars=[ydim, xdim],value_vars =['event_rate_%'], var_name = 'Type',value_name=value_name2).drop(columns='Type')
    dfl4 = dfl3.pivot(index=ydim, columns=xdim, values=value_name2)
    # flatten dfl4
    dfl4_flattened = dfl4.to_numpy().flatten(order='C')
    #print(value_name2)
    #print(dfl4)
    #print(dfl4_flattened)
    ...
    for i, p in enumerate(ax.patches):
        ...
        if 1:
            ...
            text = f'{height:.1f}%,{dfl4_flattened[i]:.1f}%'

            # annotate the bar segments
            ax.text(x+width/2, y+height/2, text, fontsize=9, horizontalalignment='center', verticalalignment='center',rotation=45)
