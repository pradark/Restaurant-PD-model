...
SIMPLIFIED VERSION: Uses IV-based feature selection instead of SHAP RFE
This avoids protobuf conflicts while maintaining the core 10-step methodology.
"""
...
# ============================================================================
# STEP 6: FEATURE SELECTION (IV-based instead of SHAP RFE)
# ============================================================================

print("\n[STEP 6/10] Feature Selection (IV-based)...")

# Select top features by IV (simplified alternative to SHAP RFE)
n_features_target = max(10, int(len(selected_iv) * 0.7))  # Keep 70% or minimum 10
selected_features = iv_df[iv_df['IV'] < 1.0].head(n_features_target)['Feature'].tolist()

print(f"  ✓ Selected {len(selected_features)} features (IV-based selection)")
print(f"    Top 5 features by IV: {iv_df.head(5)['Feature'].tolist()}")
...
# Figure 3: Feature Importance (from EBT)
...
ax.set_title('Feature Importance (EBT, Train)', fontweight='bold')
plt.tight_layout()
plt.savefig(f'{OUTPUT_DIR}/figures/fig_feature_importance.png', bbox_inches='tight', dpi=120)
plt.close()
print(f"  ✓ Saved: fig_feature_importance.png")

# Figure 4: IV Bar Chart (Alternative to SHAP)
iv_plot_df = iv_df[iv_df['Feature'].isin(selected_features)].sort_values('IV', ascending=True)
...
ax.set_title('IV-Based Feature Selection (Train)', fontweight='bold')
plt.tight_layout()
plt.savefig(f'{OUTPUT_DIR}/figures/fig_iv_importance.png', bbox_inches='tight', dpi=120)
plt.close()
print(f"  ✓ Saved: fig_iv_importance.png")
